...
Dtype PoolingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  case PoolingParameter_PoolMethod_MAX:{
    ...
    cl_int ret;
    ret=clSetKernelArg(Kernel,0,sizeof(cl_int),(void*)&top_count);
    ...
    cl_event eventPoint;
    size_t uiGlobal_Work_Size[]={top_count * 1};
    size_t uiLocal_Work_Size[]={1};
...
/*  iStatus = clEnqueueReadBuffer(amdDevice.CommandQueue,(cl_mem)top_data_, CL_TRUE, 0, (size_t)((*top)[0]->count()*sizeof(Dtype)),top_data, 0, NULL, NULL);
...
void PoolingLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  case PoolingParameter_PoolMethod_MAX:
    // The main loop
    //std::cout<<"num"<<top[0]->num()<<", channels_"<<channels_<<", pooled_height"<<pooled_height_<<", pooled_width"<<pooled_width_<<", stride"<<stride_<<std::endl;
