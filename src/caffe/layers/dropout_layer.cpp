...
void DropoutLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  //if(Caffe::mode() == Caffe::GPU){
    ...
    MaskMem = clCreateBuffer(amdDevice.Context, CL_MEM_READ_WRITE, bottom[0]->count(), NULL, NULL);
  //} 
}
...
Dtype DropoutLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (Caffe::phase() == Caffe::TRAIN) {
    ...
    clEnqueueWriteBuffer(amdDevice.CommandQueue, MaskMem, CL_TRUE, 0, count * sizeof(cl_int), (void*)mask, 0, NULL, NULL);
    cl_int ret;
    ...
    clEnqueueCopyBuffer(amdDevice.CommandQueue, (cl_mem)bottom_data, (cl_mem)top_data, 0, 0, count*sizeof(Dtype), 0, NULL, NULL);
    //caffe_copy(bottom[0]->count(), bottom_data, top_data);
  }
  return Dtype(0);
}
